...
# Clearing the varaible from the environment
rm(list = ls())
...
MergedDeals <- sqldf("SELECT * FROM MergedDeals WHERE Broker_City
                             IN (SELECT Broker_City FROM MergedDeals GROUP BY Broker_City HAVING COUNT(*) > 4) 
                     ORDER BY Broker_City")

#MergedDeals <- sqldf("SELECT * FROM MergedDeals WHERE Insured_City
 #                            IN (SELECT Insured_City FROM MergedDeals GROUP BY Insured_City HAVING COUNT(*) > 7) 
...
#MergedDeals <- CreateDummies(MergedDeals,MergedDeals$Insured_City)
#MergedDeals <- CreateDummies(MergedDeals,MergedDeals$Broker_City)
...
TunedPredictionValues <- fineTuneModel(logitRegModelPrediction,val,.20)

confusionMatrx = table(TunedPredictionValues,val$target)
confusionMatrx
...
TunedPredictionValues <- fineTuneModel(randomForestfitPrediction,val,.10)

confusionMatrx3 = table(TunedPredictionValues,val$target)
confusionMatrx3
...
TunedPredictionValues <- fineTuneModel(xgBoostModelfitPrediction,val,.10)

confusionMatrx4 = table(TunedPredictionValues,val$target)
confusionMatrx4
